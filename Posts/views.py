...
def formulario_posteo(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            posteo = Post(titulo=data ['titulo'], ingredientes=data ['ingredientes'], texto=data['texto'], tiempo=data['tiempo'], porciones=data['porciones'], autor=data['autor'], imagen=data['imagen_post'])
            posteo.save()
            return render(request, "Indice/Plantillas/carga.html", {'msj':f'Se cargo la receta "{msj}"', 'user_avatar':buscar_url_avatar(request.user)})
        else:
